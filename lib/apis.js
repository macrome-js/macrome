...
class Api {
    ...
    async write(path, content, options = {}) {
        ...
        await promises_1.mkdir(path_1.dirname(path), { recursive: true });
        const file = {
            header: {
                annotations,
            },
    ...
    async __generate(path, deps, cb) {
        for (const dep of iter_tools_es_1.objectValues(deps)) {
            errawr_1.invariant(dep instanceof Promise, 'deps argument to api.generate must be {[key]: string => Promise}');
        }
        let content = null;
        try {
            const resolvedDeps = {};
            for (const [name, dep] of iter_tools_es_1.objectEntries(deps)) {
...
class MapChangeApi extends GeneratorApi {
    ...
    async __generate(path, deps, cb) {
        ...
        let handle;
        try {
            handle = await promises_1.open(path, 'r');
            ...
            if (targetGeneratedFrom) {
                ...
                if (this.resolve(change.path) === path_1.resolve(path_1.dirname(this.resolve(path)), fromPath) &&
