...
export class Macrome {
  ...
  queue: Queue<{ change: Change; cacheEntry: CacheEntry | undefined }> | null = null;
  ...
  async enqueue(change: Change): Promise<void> {
    const { path } = change;
    const cacheEntry = fsCache.get(path) || undefined;
  ...
  async __enqueue(change: Change): Promise<void> {
    ...
    if (change.exists) {
      ...
      if (generatedFrom && !fsCache.has(generatedFrom)) {
        if (!progressive) {
  ...
  async build(): Promise<void> {
    ...
    for (const { path } of changes) {
      // This assumes I never cache anything with annotations before I've generated it
      // Which means that sometimes I will read annotations and discard them
      if (!fsCache.has(path)) {
        logger.warn(`Removing stale generated file ${path}`);
        await unlink(this.resolve(path));
