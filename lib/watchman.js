...
class WatchmanClient extends fb_watchman_1.Client {
    ...
    __expressionFrom(asymmetric) {
        ...
        const excludeExpr = compoundExpr('not', ...iter_tools_es_1.map(dirExpr, matchable_1.asArray(exclude)));
        const includeExpr = [...iter_tools_es_1.map(fileExpr, matchable_1.asArray(include))];
        ...
        return [
            'allof',
            ['type', 'f'],
            ...iter_tools_es_1.when(excludeExpr, [excludeExpr]),
            ...iter_tools_es_1.when(includeExpr, includeExpr),
            // See https://facebook.github.io/watchman/docs/expr/suffix.html#suffix-set
            ...iter_tools_es_1.when(suffixExpr, [suffixExpr]),
        ];
    }
    ...
    async query(path, expression, options) {
        ...
        const resp = await this.command('query', this.watchRoot, Object.assign(Object.assign(Object.assign({}, options), { relative_root: path_1.relative(this.watchRoot, path_1.join(this.root, path)) }), iter_tools_es_1.when(expression, { expression: this.__expressionFrom(expression) })));
...
const getWatchmanIgnoreDirs = async (root) => {
    const watchmanConfigPath = await find_up_1.default('.watchmanconfig', { cwd: root });
    if (!watchmanConfigPath)
        return;
    const watchmanConfig = JSON.parse(await promises_1.readFile(watchmanConfigPath, 'utf-8'));
    const ignoreDirs = watchmanConfig.ignore_dirs;
    const rootRelative = path_1.relative(path_1.dirname(watchmanConfigPath), root);
    if (!ignoreDirs || !ignoreDirs.length)
        return;
    return new Set(ignoreDirs.map((path) => path_1.join(rootRelative, path.endsWith('/') ? path : `${path}/`)));
};
...
async function standaloneQuery(root, expression) {
    ...
    const shouldExclude = matchable_1.mergeExcludeMatchers(watchmanIgnoreMatcher, makeExcludeMatcher(exclude));
    return await iter_tools_es_1.execPipe(fs_1.recursiveReadFiles(root, { shouldInclude, shouldExclude }), iter_tools_es_1.asyncFlatMap(async (path) => {
        try {
            const stats = await promises_1.stat(path_1.join(root, path));
            return [
                {
                    op: 'A',
                    path,
