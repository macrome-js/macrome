...
class Macrome {
  constructor(options = {}) {
    ...
    const stubs = this.options.generators.map((generator) => {
      let path;
      let options_;
      if (Array.isArray(generator)) {
        [path, options_] = generator;
      } else {
        path = generator;
        options_ = {};
      }

      const options = { logger: this.options.logger, ...options_ };
  ...
  async watch() {
    const { vcsConfig, vcsRoot, logger } = this;
    const { alwaysIgnored: ignored } = this.options;
    const client = new WatchmanClient(this.projectRoot, { logger });
    ...
    await client.subscribe(
      ...
      'macrome-vcs-lock',
      {
        matchable: { files: [join(vcsConfig.dir, vcsConfig.lock)] },
        fields: ['name', 'exists'],
        defer_vcs: false,
      },
      async (files) => {
