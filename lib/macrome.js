...
class Macrome {
    ...
    async __instantiateGenerators(generatorPath) {
        ...
        for (const stub of stubs) {
            ...
            const api = apis_1.GeneratorApi.fromApi(this.api, path_1.relative(this.watchRoot, generatorPath));
    ...
    async clean() {
        ...
        for (const change of changes) {
            if (change.op !== 'D' && change.annotations != null) {
                await promises_1.unlink(this.resolve(change.reported.path));
            }
        }
    }
    enqueue(change) {
        ...
        if (op === 'D') {
            ...
            if ((state === null || state === void 0 ? void 0 : state.mtimeMs) === reported.mtimeMs) {
                ...
                const generatedFrom = annotations === null || annotations === void 0 ? void 0 : annotations.get('generatedfrom');
                if (generatedFrom && ((_a = state === null || state === void 0 ? void 0 : state.annotations) === null || _a === void 0 ? void 0 : _a.get('generatedFrom')) === generatedFrom) {
                ...
                if (change.op !== 'D') {
                    await this.__forMatchingGenerators(path, async (generator, { mappings, api: genApi }) => {
                        ...
                        try {
                            ...
                            logger.error(errawr_1.Errawr.print(new errawr_1.Errawr(errawr_1.rawr(`Error mapping {path}`)({
                                path,
            ...
            for (const generator of this.generatorInstances) {
                if (generatorsToReduce.has(generator)) {
                    ...
                    try {
                        ...
                        logger.error(errawr_1.Errawr.print(new errawr_1.Errawr(errawr_1.rawr(`Error reducing {generator}`)({
                            generator: api.generatorPath,
                        }), { cause: error })));
                    }
                }
        ...
        for (const change of changes) {
            ...
            if (change.op !== 'D' && change.annotations) {
                if (!this.state.has(reported.path)) {
                    await promises_1.unlink(this.resolve(reported.path));
                }
            }
        }
        await this.processChanges();
    ...
    async watch() {
        ...
        await this.__build(await iter_tools_es_1.execPipe(changes, iter_tools_es_1.asyncMap(async (change) => await this.__decorateChangeWithAnnotations(change)), iter_tools_es_1.asyncToArray));
