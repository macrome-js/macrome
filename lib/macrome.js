...
class Macrome {
    ...
    async enqueue(change) {
        const { path } = change;
        const cacheEntry = fs_cache_1.fsCache.get(path) || undefined;
    ...
    async __enqueue(change) {
        ...
        if (change.exists) {
            ...
            if (generatedFrom && !fs_cache_1.fsCache.has(generatedFrom)) {
                if (!progressive) {
    ...
    async build() {
        ...
        for (const { path } of changes) {
            ...
            if (!fs_cache_1.fsCache.has(path)) {
                logger_1.logger.warn(`Removing stale generated file ${path}`);
                await unlink(this.resolve(path));
                await this.enqueue({ path, exists: false });
            }
        }
