    ...
    async processChanges() {
        ...
        while (true) {
            ...
            for (const change of queue) {
                const { path } = change;
                const cacheItem = fs_cache_1.fsCache.get(path);
                ...
                if (!change.exists) {
                    ...
                    for (const prevPath of prevGeneratedPaths) {
                        ...
                        if ((await this.readAnnotations(prevPath)) !== null) {
        ...
        await client.subscribe(this.root, 'macrome-main', expression, {
            ...
            if (this.queue === null) {
                ...
                for (const change of changes) {
                    ...
                    if (change.exists ? ((_a = fs_cache_1.fsCache.get(path)) === null || _a === void 0 ? void 0 : _a.mtimeMs) !== change.mtimeMs : fs_cache_1.fsCache.has(path)) {
                        await this.enqueue(change);
                    }
                }
            }
