...
class Macrome {
    ...
    async __instantiateGenerators(generatorPath) {
        ...
        for (const { options, path } of stubs) {
            ...
            const api = apis_1.GeneratorApi.fromApi(this.api, /^[./]/.test(path) ? `/${(0, path_1.relative)(this.watchRoot, generatorPath)}` : path);
    ...
    __enqueue(change) {
        ...
        if (!(annotations === null || annotations === void 0 ? void 0 : annotations.has('generatefailed'))) {
            logger.debug(`enqueueing ${verbFor(change)} {path: ${path}}` +
    ...
    async processChanges() {
        ...
        while (queue.size) {
            ...
            while (queue.size) {
                ...
                logger.debug(`executing ${verbFor(change)} {path: ${path}}` +
                    (reported.op !== 'D' ? ` modified at {mtimeMs: ${reported.mtimeMs}}` : ''));
                if (change.op !== 'D') {
                    await this.__forMatchingGenerators(path, async (generator, { mappings, api: genApi }) => {
                        ...
                        try {
                            ...
                            const mapResult = generator.map
                                ? await Promise.race([generator.map(api, change), (0, timer_1.timeout)(5000)])
                                : change;
                            mappings.set(path, mapResult);
                        }
                        catch (error) {
                            const rootCause = (0, iter_tools_es_1.takeLast)(errawr_1.Errawr.chain(error));
                            if ((rootCause === null || rootCause === void 0 ? void 0 : rootCause.code) === 'macrome_would_overwrite_source') {
                                ...
                                throw new errawr_1.Errawr((0, errawr_1.rawr)(`Error mapping {path}`), {
            ...
            for (const generator of this.generatorInstances) {
                if (generatorsToReduce.has(generator) && generator.reduce) {
                    ...
                    try {
                        await Promise.race([generator.reduce(api, mappings), (0, timer_1.timeout)(10000)]);
                    }
                    catch (error) {
                        throw new errawr_1.Errawr((0, errawr_1.rawr)(`Error reducing {generator}`)({
    ...
    relative(path) {
        return (0, path_1.relative)(this.root, this.resolve(path));
    }
    resolve(path) {
        return path.startsWith('/') ? path : (0, path_1.resolve)(this.root, path);
    }
}
