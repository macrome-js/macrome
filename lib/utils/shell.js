...
function output(cmd, args, dir = process.cwd()) {
  return run(cmd, args, dir).stdout.toString();
}

function outputLines(cmd, args, dir = process.cwd()) {
  return output(cmd, args, dir).split(/\r?\n/g);
}
...
module.exports = { run, output, outputLines, hasOutput };
